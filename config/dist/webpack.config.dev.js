...
var appPackageJson = require(paths.appPackageJson); // Source maps are resource heavy and can cause out of memory issue for large source files.
// const shouldUseSourceMap = process.env.GENERATE_SOURCEMAP !== 'false';


var shouldUseSourceMap = false; // Some apps do not need the benefits of saving a web request, so not inlining the chunk
// makes for a smoother build process.

var shouldInlineRuntimeChunk = process.env.INLINE_RUNTIME_CHUNK !== 'false';
var imageInlineSizeLimit = parseInt(process.env.IMAGE_INLINE_SIZE_LIMIT || '10000'); // Check if TypeScript is setup
...
var _getPackageJson = getPackageJson('version', 'name', 'license', 'repository', 'author'),
    version = _getPackageJson.version,
    name = _getPackageJson.name,
    license = _getPackageJson.license,
    repository = _getPackageJson.repository,
    author = _getPackageJson.author;

var banner = "\n  ".concat(name, " v").concat(version, "\n  ").concat(repository.url, "\n\n  Copyright (c) ").concat(author.replace(/ *\<[^)]*\> */g, " "), "\n\n  This source code is licensed under the ").concat(license, " license found in the\n  LICENSE file in the root directory of this source tree.\n  "); // This is the production and development configuration.
// It is focused on developer experience, fast rebuilds, and a minimal bundle.

module.exports = function (webpackEnv) {
  var isEnvDevelopment = webpackEnv === 'development';
  var isEnvProduction = webpackEnv === 'production'; // Webpack uses `publicPath` to determine where the app is being served from.
  // It requires a trailing slash, or the file assets will get an incorrect path.
  // In development, we always serve from the root. This makes config easier.

  var publicPath = isEnvProduction ? paths.servedPath : isEnvDevelopment && '/'; // Some apps do not use client-side routing with pushState.
  // For these, "homepage" can be set to "." to enable relative asset paths.

  var shouldUseRelativeAssetPaths = publicPath === './'; // `publicUrl` is just like `publicPath`, but we will provide it to our app
  // as %PUBLIC_URL% in `index.html` and `process.env.PUBLIC_URL` in JavaScript.
  // Omit trailing slash as %PUBLIC_URL%/xyz looks better than %PUBLIC_URL%xyz.

  var publicUrl = isEnvProduction ? publicPath.slice(0, -1) : isEnvDevelopment && ''; // Get environment variables to inject into our app.

  var env = getClientEnvironment(publicUrl); // common function to get style loaders
  ...
  return {
    ...
    entry: isEnvProduction ? [paths.appLibIndexJs] : [// Include an alternative client for WebpackDevServer. A client's job is to
    ...
    output: {
      ...
      filename: isEnvProduction ? 'index.js' : isEnvDevelopment && 'static/js/bundle.js',
      ...
      devtoolModuleFilenameTemplate: isEnvProduction ? function (info) {
        return path.relative(paths.appSrcLib, info.absoluteResourcePath).replace(/\\/g, '/');
      } : isEnvDevelopment && function (info) {
        return path.resolve(info.absoluteResourcePath).replace(/\\/g, '/');
      },
      // Prevents conflicts when multiple Webpack runtimes (from different apps)
      // are used on the same page.
      jsonpFunction: "webpackJsonp".concat(appPackageJson.name)
    },
    optimization: {
    ...
    plugins: [new webpack.ProvidePlugin({// $: "jquery",
      ...
      // "window.$": "jquery"
    }), isEnvDevelopment && // Generates an `index.html` file with the <script> injected.
    ...
    useTypeScript && new ForkTsCheckerWebpackPlugin({
      ...
      resolveModuleNameModule: process.versions.pnp ? "".concat(__dirname, "/pnpTs.js") : undefined,
      resolveTypeReferenceDirectiveModule: process.versions.pnp ? "".concat(__dirname, "/pnpTs.js") : undefined,
